...
public class _Reversi {
/////////************  Variables   ********////////
	private Board board=new Board();
	
	
//////////// Transfered to Class board///////////////////
...
////*********** Essai function **************////////
	public boolean isValidmove(){
		for(int i=0;i<8;i++){
			for(int j=0;j<8;j++){
				if(this.board.getCells()[i][j].isEmpty()){
					if(isEnemyNearby("B",i,j)){
...
///***************Function to get the new output****************////////	
	public String newOutput(){
		String output="";
		for(int i=0;i<8;i++){
			for(int j=0;j<8;j++){	 
				output=output+this.board.getCells()[i][j].getContent()+" ";
			}
			output=output+"\n";
		}
		output=output+"B";
